function [net, info] = cnn_cifar_train_b1(varargin)
...
opts.expDir = fullfile('cifar', 'cifar10_block_1') ;
[opts, varargin] = vl_argparse(opts, varargin) ;

opts.train.numEpochs = numel(opts.train.learningRate) ;
[opts, varargin] = vl_argparse(opts, varargin) ;

opts.dataDir = fullfile('data','cifar-10') ;
opts.imdbPath = fullfile(opts.dataDir, 'cifar-imdb.mat');
...
switch opts.modelType
  case 'lenet', 
	if opts.random
		net = cnn_cifar_init_random(opts) ;
	else
		net = cnn_cifar_init_pretrain(opts) ;
	end
  case 'nin',   
...
[net, info] = cnn_train_b1(net, imdb, @getBatch, ...
